...
export async function POST(req: NextRequest) {
  try {
    ...
    if (mood.trim().length > 300) {
      return NextResponse.json({ error: "Mood description too long" }, { status: 400 });
    }
    ...
    const genreId = genreMap[parsed.genreName] ?? "";

    const validSortOptions = [
      "popularity.desc", "vote_average.desc", "release_date.desc", "vote_count.desc",
    ];
    const sortBy = validSortOptions.includes(parsed.sortBy)
      ? parsed.sortBy
      : "popularity.desc";
    ...
    const moodLabel =
      typeof parsed.moodLabel === "string"
        ? parsed.moodLabel.slice(0, 50)
        : mood;
